...
export = VehicleOysterSauceStatistics;

class VehicleOysterSauceStatistics extends BaseWidget {
    baseClass = "widget-VehicleOysterSauceStatistics";
    toast = null;
    popup = null;
    loadWait=null;
    companyid = "";//公司id
    ...
    initTableHtml(){
        if(this.iszgs){
            this.domObj.find(".oystersaucestatisticscaption").empty().append('<div class="cxgs" style="float:left;">公司：</div><div class="cxqj" style="float:right;">查询日期：</div>'); 
        }else{
            this.domObj.find(".oystersaucestatisticscaption").empty().append('<div class="cxqj" style="float:left;">查询日期：</div>');            
        }
    }
    ...
    initEvent() { 
        ...
        this.domObj.find('.btn_print').on("click", function () {           
            (<any>$(".oystersaucestatistics-list")).print({                    
                    globalStyles: false,
                    mediaPrint: false,
                    stylesheet: "widgets/VehicleOysterSauceStatistics/css/style.css",
                    iframe: true,
     ...
     * @function 加油记录统计
     */
    getReportCarFuelConsume() {
        this.loadWait.Show("正在查询数据，请耐心等待...",this.domObj);
        $.ajax({
            headers: this.getHeaders(0),
            type: "POST",
            url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + this.config.getReportCarFuelConsume,
            data: {
            ...
            success: this.getReportCarFuelConsumeCallback.bind(this),
            error: function (data) {
                this.loadWait.Hide();
    ...
    getReportCarFuelConsumeCallback(results) {
        this.loadWait.Hide();
        var that = this;
        if (results.code != 1) {
            that.toast.Show(results.message);
        ...
        if(data!=null && data.length>0){
            for(var i=0;i<data.length;i++){
                this.html+="<tr><td>"+data[i].dept
                        +"</td><td>"+data[i].car_count
                        +"</td><td>"+data[i].total_fuel_num
        ...
        this.domObj.find(".oystersaucestatisticslist").empty().append(this.html); 
        var myDate = new Date();
