...
class VehicleMouthReport extends BaseWidget {
    ...
    getVehicleMouthReport() {
        ...
        $.ajax({
            ...
            type: "POST",
            url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + this.config.getVehicleMouthReport,
            data: {
                "q_mouth_date": this.search_obj.mouth_date,
                "carid": this.search_obj.carid,
                "companyid": this.search_obj.depid
            },
            success: this.getVehicleMouthReportCallback.bind(this),
            error: function (data) {
                this.loadWait.Hide();
    ...
    getVehicleMouthReportCallback(results) {
        this.loadWait.Hide();
        console.log(results);
        var that = this;
        if (results.code != 1) {
        ...
        this.domObj.find(".vehiclemouthreportlist").empty().append(this.html); 
        var myDate = new Date();
