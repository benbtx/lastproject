...
export = TufuhaoQuery;
class TufuhaoQuery extends BaseWidget {
    baseClass = "tufuhaoQuery";
    ...
    tufuhaoQueryInit() {
        this.domObj.on('keydown', 'input', this.onInputKeyDown.bind(this));
        if (this.AppX.appConfig.gisResource.pipe.config.length == 0) {
            ...
            //查询按钮 添加绑定事件
            $(".tufuhaoQuery-queryBtn").bind("click", this.queryBtnCallback.bind(this));
            var maxLayerCount = layers.filter(item => { return item.urlindex == 0 }).length;
            this.limitSelect(".tufuhaoQuery-layerName", maxLayerCount);
        }
    }
    ...
    addOptionToDrapdow(layers) {
        ...
        for (var i = 0, length = layers.length; i < length; i++) {
            ...
            $(option).appendTo($(".tufuhaoQuery-layerName"));
    ...
    queryBtnCallback(event) {
        ...
        var inputObj = $(".tufuhaoQuery input");
        if (inputObj.val() == "") {
            inputObj.attr("placeholder", "不能为空！");
            return;
        }
        if ($(".tufuhaoQuery-queryBtn").text() == '查询中...') {
            return;
        }
        $(".tufuhaoQuery-queryBtn").text('查询中...');
        var SQLWhere: string = inputObj.val();
        SQLWhere = "DISTNO like" + "'%" + SQLWhere + "%'";

        var selectedObj = $(".tufuhaoQuery-layerName").find("option:selected");
        //获取选中图层的id
