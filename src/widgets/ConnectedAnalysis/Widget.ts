...
export = ConnectedAnalysis;

class ConnectedAnalysis extends BaseWidget {
    baseClass = "widget-connected-analysis";
    debug = false;
    ...
    */
    shortStack() {
        ...
        if (this.AppX.appConfig.gisResource.apiconnectedpipe.config.length > 0) {
            this.analyzeUrl = this.AppX.appConfig.gisResource.apiconnectedpipe.config[0].url;
        } else {
            this.AppX.runtimeConfig.toast.Show('获取连通性分析服务出错！');
        }
    ...
    */
    bindElement() {
        ...
        this.btnApplyStop = this.domObj.find('.applystop');
    }
    ...
    */
    initEvents() {
        this.domObj.on('click', '.pick-pipe', this.pickPipe.bind(this))
            ...
            .on('click', '.applystop', this.onApplystopClick.bind(this));
    }
  ...
  */
    private submitAnalyzeTask(layerdbname, oid) {
        ...
        var applystop = true;
        if (!this.btnApplyStop.hasClass('glyphicon-check'))
            applystop = false;
        var options = {
    ...
    */
    private onAnalyzeComplete(jobinfo) {
        ...
        if (jobinfo.jobStatus == "esriJobSucceeded") {
            ...
            var resultMaptUrl = gpBaseUrl + jobid;
            ...
            this.getAnalyzsSummary(this.analyzeUrl + "/jobs/" + jobid + "/" + summaryUrl);
        } else {
    ...
    */
    ...
    /**
      ...
      * @return {(返回值类型)} (返回值说明)
      */
    private findOidsofLayer(mapurl, layerdbname) {
    ...
    private findOidsofLayerError(error) {
        console.error(error);
    }
    private queryDetailofLayer(response) {
        if (response && response.length > 0) {
            var oids: number[] = [];
            var layername;
            response.forEach(
                function (value, index) {
                    oids.push(parseInt(value.feature.attributes["sourceoid"]));
                    layername = value.feature.attributes["layername"];
                }.bind(this)
            );
            var urlAndLayerid = this.getSubLayerIdofPipeMap(layername);
            if (urlAndLayerid == null)
                return;
            var querylayerid = parseInt(urlAndLayerid.split(':')[1]);
            var urlindex = parseInt(urlAndLayerid.split(':')[0]);
            var queryUrl = this.AppX.appConfig.gisResource.pipe.config[urlindex].url + "\/" + querylayerid;
            ...
            queryTask.execute(query).then(
                function (response) {
                    ...
                    var tab = this.dataPanelWrap(urlindex, response, layername, querylayerid,dataQuery);
                    this.resultSummary[this.detailFeild].push(tab);
                    if (this.resultSummary[this.detailFeild].length == this.resultSummary.layersummary.length) {
                        this.AppX.runtimeConfig.dataPanel.show({ tabs: this.resultSummary[this.detailFeild] });
                        this.btnViewDetails.removeClass('disabled');
                    }
                }.bind(this)
            ).otherwise(this.queryDetailErrorCallBack.bind(this));
        }
    }
...
*/
    ...
    */
    cleanAll() {
        ...
        if (this.gp != null && this.jobid) {
            this.gp.cancelJob(this.jobid, function () { console.log("job canceled!") }.bind(this), function () { console.log("job cancel fail!") }.bind(this));
        }
    }
