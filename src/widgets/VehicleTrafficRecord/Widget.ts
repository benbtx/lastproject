...
export = VehicleTrafficRecord;

class VehicleTrafficRecord extends BaseWidget {
    baseClass = "widget-VehicleTrafficRecord";
    toast = null;
    popup = null;
    loadWait=null;
    currentPopup=null;
     ...
     * @param person_id 责任人
     */
    setSearchVal(depid,carid,start_date,end_date,person_id){
        if(this.ispartment==false)
            this.depid=depid;
        this.carid=carid;
    ...
    initEvent() {
        ...
        this.domObj.find('.btn_search').on("click", function () {           
            this.config.currentpage=1;
            this.setSearchVal(this.domObj.find(".department option:selected").val(),this.domObj.find(".carids option:selected").val(),this.domObj.find(".start_date").val(),this.domObj.find(".end_date").val(),this.domObj.find(".userids option:selected").val());
            this.getVehicleTrafficRecord();
        }.bind(this));
        ...
        this.domObj.find('.btn_delete').on("click", function () {
            ...
            Obj.submitObj.off("click").on("click", function () {
                ...
                $.ajax({
                    ...
                    type: "POST",
                    dataType: "json",
                    url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + this.config.deleteCarTrafficAccident,
                    data: {
        ...
        //全选
        this.domObj.find('.select_trafficinfo_all_list').off("change").on("change", function () {
            if (this.domObj.find(".select_trafficinfo_all_list").prop('checked') == true) {
                this.domObj.find(".select_trafficinfo_list").each(function () {                    
    ...
    getVehicleTrafficRecord() { 
        this.domObj.find('.select_trafficinfo_all_list').prop("checked",false);  
        ...
        $.ajax({
            ...
            type: "POST",
            dataType: "json",
            url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + this.config.getCarTrafficAccidentList,
            data: {
                "pageindex": this.config.currentpage,
                "pagesize": this.config.pagesize,
                "blameperson":this.person_id,
                "accidentcar":this.carid,
                "deptid":this.depid,
                "accidentstartdate":this.start_date,
                "accidentenddate":this.end_date,
                "companyid":this.companyid
            },
            success: this.getVehicleTrafficRecordListCallback.bind(this),
            error: function (data) {
                this.loadWait.Hide();
    ...
    getVehicleTrafficRecordListCallback(results) {
        console.log(results);
        this.loadWait.Hide();     
        var that = this;
        if (results.code != 1) {
        ...
        $.each(results.result.rows, function (i, item) {
            var accident_date= (item.accident_date!=null &&item.accident_date!=""?Functions.DateFormat(new Date(item.accident_date.replace(/-/g,"/")),"yyyy-MM-dd hh:mm:ss"):"");
            html_trs_data += "<tr class=goto  data-id='" + item.id
             ...
             + "' data-blame_percent='" + item.blame_percent
             + "' data-lose_amount='" + item.lose_amount
             + "' data-insurance_percent='" + item.insurance_percent
             + "' data-blame_person='" + item.blame_person
             + "' data-blame_person_id='" + item.blame_person_id
             + "' data-personal_compensation='" + item.personal_compensation
             + "' data-notes='" + item.notes
             + "' data-process_status='" + item.process_status
             + "' data-process_result='" + item.process_result
             + "'><td class='checkwidth'><input type='checkbox' class='select_trafficinfo_list' data-id='" + item.id
             + "' data-accident_date='" + accident_date 
             + "' data-carid='" + item.carid
             + "' data-car='" + item.car
             + "' data-address='" + item.address 
             ...
             + item.car + "</td><td title='"+accident_date+"'>"           
             + accident_date + "</td><td title='"+item.address+"'>"
             + item.address + "</td><td title='"+item.cause+"'>" 
             + item.cause + "</td><td title='"+item.blame_percent+"'>" 
             + item.blame_percent + "</td><td title='"+item.lose_amount+"'>"
             + item.lose_amount + "</td><td title='"+item.insurance_percent+"'>"
             + item.insurance_percent + "</td><td title='"+item.blame_person+"'>"
             + item.blame_person + "</td><td title='"+item.personal_compensation+"'>"
             + item.personal_compensation + "</td><td title='"+item.notes+"'>"
             + item.notes + "</td><td title='"+item.process_status+"'>"
             + item.process_status + "</td><td title='"+item.process_result+"'>"
             + item.process_result              
             + "</td></tr>";           
        }.bind(this));
        this.domObj.find(".trafficinfolist").empty().append(html_trs_data);

        //勾选
        this.domObj.find('.select_trafficinfo_list').off("change").on("change",function(e){  
    ...
    showInfo(target,tit,idx){
        ...
        if(target!=null){           
            this.editlabel="edit"; 
            id=target.data("id");
            Obj.conObj.find('.accident_date').val(target.data("accident_date"));
            Obj.conObj.find('.address').val(target.data("address"));
            Obj.conObj.find('.cause').val(target.data("cause"));
            Obj.conObj.find('.blame_percent').val(target.data("blame_percent"));
            Obj.conObj.find('.lose_amount').val(target.data("lose_amount"));
            Obj.conObj.find('.insurance_percent').val(target.data("insurance_percent"));
            Obj.conObj.find('.personal_compensation').val(target.data("personal_compensation"));
        ...
        (<any>$('#widget-VehicleTrafficRecord_addpopup')).bootstrapValidator();
        //提交保存
        Obj.submitObj.off("click").on("click", function () {
                var blame_person_id = Obj.conObj.find('.blame_person_id option:selected');
                var accident_date = Obj.conObj.find('.accident_date'); 
                var address = Obj.conObj.find('.address');
                var cause = Obj.conObj.find('.cause'); 
                var blame_percent = Obj.conObj.find('.blame_percent');  
                var lose_amount = Obj.conObj.find('.lose_amount');                                     
                var insurance_percent = Obj.conObj.find('.insurance_percent');
                var personal_compensation = Obj.conObj.find('.personal_compensation');
                var process_status = Obj.conObj.find('.process_status');
                var process_result = Obj.conObj.find('.process_result');
                if (accident_date.val() == '') {
                    accident_date.addClass('has-error');                       
                    this.popup.ShowTip("事故时间不能为空！");
                    return;
                } 
                 if (address.val() == '') {
                    address.addClass('has-error');                       
                    this.popup.ShowTip("事故地点不能为空！");
                    return;
                } 
                if (cause.val() == '') {
                    cause.addClass('has-error');                       
                    this.popup.ShowTip("事故原因不能为空！");
                    return;
                } 
                if (blame_percent.val() == '') {
                    blame_percent.addClass('has-error');                       
                    this.popup.ShowTip("责任比例不能为空！");
                    return;
                }
                if (lose_amount.val() == '') {
                    lose_amount.addClass('has-error');                       
                    this.popup.ShowTip("损失金额不能为空！");
                    return;
                }else{
                    if(isNaN(lose_amount.val())){
                         lose_amount.addClass('has-error');                       
                         this.popup.ShowTip("损失金额填写不正确，必须为数字！");
                         return;
                    }
                }
                if (insurance_percent.val() == '') {
                    insurance_percent.addClass('has-error');                       
                    this.popup.ShowTip("出险比例不能为空！");
                    return;
                } 
                if (personal_compensation.val() == '') {
                    personal_compensation.addClass('has-error');                       
                    this.popup.ShowTip("个人赔付金额不能为空！");
                    return;
                }else{
                    if(isNaN(personal_compensation.val())){
                        this.popup.ShowTip("个人赔付金额填写不正确，必须为数字！");
                        return;
                    }
                }
                ...
                $.ajax({
                    ...
                    type: "POST",
                    dataType: "json",
                    url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + (this.editlabel=="add"?this.config.addCarTrafficAccident:this.config.updateCarTrafficAccident),
                    data: {
                        "id":id,
                        ...
                        "address":address.val(),
                        "cause":cause.val(),
                        "blame_percent":blame_percent.val(),
                        "lose_amount":lose_amount.val(),
                        "insurance_percent":insurance_percent.val(),
                        "personal_compensation":personal_compensation.val(),
                        "process_status":process_status.val(),
                        "process_result":process_result.val(),
                        "blame_person_id":Obj.conObj.find('.blame_person_id option:selected').val(),
    ...
    getCarList(domObj,isall,controlname){
        $.ajax({
                ...
                success: function (result) {
                    if (result.code != 1) {
                        this.popup.ShowTip(result.message);
                        return;
                    }
     ...
     * @param isall
     */
    getUser(selectClass,domObj,isall) {
        $.ajax({
            headers: this.getHeaders(0),
            type: "POST",
            dataType: "json",
            ...
            success: function(results){
                ...
                domObj.find("."+selectClass).empty().append(strusers);
            }.bind(this),
            error: function (data) {
                this.toast.Show(this.config.messages.other);
            }.bind(this)
    ...
    getPerson() {
        $.ajax({
            ...
            success: this.getPersonListCallback.bind(this),
            error: function (data) {
                this.toast.Show(this.config.messages.other);
            }.bind(this)
        });
    }
    getPersonListCallback(results) {
        var that = this;
        if (results.code != 1) {
            that.toast.Show(results.message);
            return;
        ...
        var strusers = "";
        $.each(results.result.rows, function (index, item) {
            if(this.select_info.currentTarget!=null && this.select_info.currentTarget.data("blame_person_id")==item.userid && this.editlabel=="edit"){
                strusers += "<option selected value=" + item.userid + " > " + item.username + " </option>";
            }else{
                strusers += "<option value=" + item.userid + " > " + item.username + " </option>";
            }            
        }.bind(this));
        this.currentPopup.conObj.find(".blame_person_id").empty().append(strusers);
    } 
