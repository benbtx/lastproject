...
export = PhotoTypeManagement;

class PhotoTypeManagement extends BaseWidget {
    baseClass = "widget-phototypemanagement";
    ...
    initHtml() {
        ...
        this.setHtml(html);
        this.ready();
        this.getPhotoType(this.config.pagenumber);
    ...
    getPhotoTypeListCallback(results) {
        ...
        $.each(results.result.rows, function (i, item) {
            ...
            html_trs_data += "<tr class=goto  data-id='" + item.id + "' data-value='" + item.value + "' data-notes='" + item.notes + "' data-category='" + item.category + "' data-default_level='" + item.default_level + "'><td>" + item.value + "</td><td>" + item.notes + "</td></tr>";
            //刷新infowindow
    ...
    initEvent() {
        ...
        this.domObj.find('.btn_add').on("click", function () {
            ...
            //验证
            (<any>$('#phototypemanagement_addpopup')).bootstrapValidator();

            Obj.submitObj.off("click").on("click", function () {

                var value = Obj.conObj.find(".value");
                ...
                $.ajax({
                    ...
                    type: "POST",
                    url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + this.config.addPhotoType,
                    data: {
                        "key": "0011",
                        "value": value.val(),
                        "notes": notes.val(),
                    ...
                    success: this.addPhotoTypeCallback.bind(this),
                    error: function (data) {
        ...
        this.domObj.find('.btn_edit').on("click", function () {
            ...
            Obj.conObj.find('.value').val(this.value);
            ...
            //验证
            (<any>$('#phototypemanagement_updatepopup')).bootstrapValidator();
            ...
            Obj.submitObj.off("click").on("click", function () {
                ...
                $.ajax({
                    ...
                    type: "POST",
                    url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + this.config.updatePhotoType,
                    data: {
                    ...
                    success: this.updatePhotoTypeCallback.bind(this),
                    error: function (data) {
                        this.toast.Show("服务端ajax出错，获取数据失败!");
                    }.bind(this),
                    dataType: "json",
        ...
        this.domObj.find('.btn_delete').on("click", function () {
            ...
            Obj.submitObj.off("click").on("click", function () {
                var arr = [];
                arr.push(this.hiddendangertypeid);
                $.ajax({
                    headers: {
                    ...
                    type: "POST",
                    url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + this.config.deletePhotoType,
                    data: {
                        // "id": this.hiddendangertypeid,
                        "list": arr,
                        // f: "pjson"
                    },
                    success: this.deletePhotoTypeCallback.bind(this),
                    error: function (data) {
                        this.toast.Show("服务端ajax出错，获取数据失败!");
                    }.bind(this),
                    dataType: "json",
        ...
        this.domObj.find(".pre").off("click").on("click", function () {
            if (this.currentpage - 1 > 0) {
                ...
                this.getPhotoType(this.currentpage);
    ...
    updatePhotoTypeCallback(results) {
    ...
    deletePhotoTypeCallback(results) {
