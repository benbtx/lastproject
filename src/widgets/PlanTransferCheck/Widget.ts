...
export = PlanTransferCheck;

class PlanTransferCheck extends BaseWidget {
    baseClass = "widget-plantransfercheck";
    map = null;
    toast = null;
    popup = null;
    totalpage = 1;
    ...
    // currentpagesieze = 1;
    // currentpagecount = 0;
    curentplantransferchecktypeid = "";
    plantransfercheckInfoGraphicLayer: GraphicsLayer;
    jsongeometry = "";
    editgeometry: Point;
    editid = "";
    polygonid = "";
    ...
    // plan_begindate = "";
    // plan_enddate = "";
    plantransfercheck_planpoint_clusterLayer = null;//巡检点
    plantransfercheck_devicepoint_clusterLayer = null;//设备点
    regionid = "";
    ...
    plandetail_polylinelayer_gl: GraphicsLayer;
    plandetail_pointlayer_gl: GraphicsLayer;
    ...
    initHtml() {
        ...
        //查询计划列表
        this.showLoading();
        // this.getAllpersonPlanTransferCheck(this.config.pagenumber, this.config.pagesize, "", "", "");
        this.getPlanTransferCheck(this.config.pagenumber, this.config.pagesize, this.region_id, this.depid, this.user_id, this.plan_begindate, this.plan_enddate);
    ...
    configure() {
        ...
        if (this.map.getLayer("plantransfercheck")) {
            return;
        } else {
            var graphicLayer = new GraphicsLayer();
            graphicLayer.id = "plantransfercheck";
            this.plantransfercheckInfoGraphicLayer = graphicLayer;
        ...
        //巡检线——高亮类型
        if (this.map.getLayer("plandetail_polylinelayer_gl")) {
            return;
        } else {
            var plandetail_polylinelayer_gl = new GraphicsLayer();
            plandetail_polylinelayer_gl.id = "plandetail_polylinelayer_gl";
            this.plandetail_polylinelayer_gl = plandetail_polylinelayer_gl;
            this.map.addLayer(plandetail_polylinelayer_gl);
        }

        //巡检点——高亮类型
        if (this.map.getLayer("plandetail_pointlayer_gl")) {
            return;
        } else {
            var plandetail_pointlayer_gl = new GraphicsLayer();
            plandetail_pointlayer_gl.id = "plandetail_pointlayer_gl";
            this.plandetail_pointlayer_gl = plandetail_pointlayer_gl;
            this.map.addLayer(plandetail_pointlayer_gl);
        }
        ...
        if (this.map.getLayer("planregionInfoGraphicLayer")) {
            return;
        } else {
            var planregionInfoGraphicLayer = new GraphicsLayer();
            planregionInfoGraphicLayer.id = "planregionInfoGraphicLayer";
            this.planregionInfoGraphicLayer = planregionInfoGraphicLayer;
            this.map.addLayer(planregionInfoGraphicLayer);
        }
    ...
    initEvent() {
        ...
        this.domObj.find(".pre").off("click").on("click", function () {
            if (that.currentpage - 1 > 0) {
                ...
                // that.getPlanTransferCheck(that.currentpage, that.config.pagesize, "");
                that.showLoading();
                // that.getAllpersonPlanTransferCheck(that.currentpage, this.currentpagesize, that.depid, that.step1_start, that.step1_end);
                // that.getPlanTransferCheck(that.currentpage, this.currentpagesize, this.user_id);
                that.getPlanTransferCheck(that.currentpage, this.currentpagesize, this.region_id, this.depid, this.user_id, this.plan_begindate, this.plan_enddate);
        ...
        //     this.initEvent2();
        //     //查询某个人计划列表
        //     this.getPlanTransferCheck(this.config.pagenumber, this.config.pagesize, this.user_id);
        ...
        this.domObj.find(".btn-search").off("click").on("click", function () {
            ...
            this.showLoading();
            // this.getAllpersonPlanTransferCheck(this.config.pagenumber, this.config.pagesize, this.depid, this.step1_start, this.step1_end);
    ...
    initEvent2() {
        ...
        this.domObj.find('.btn_return').off("click").on("click", function () {
            ...
            this.plantransfercheckInfoGraphicLayer.clear();

            this.plandetail_polylinelayer.clear();
            this.plandetail_pointlayer.clear();
            this.plandetail_polylinelayer_gl.clear();
            this.plandetail_pointlayer_gl.clear();
            ...
            // this.domObj.append(this.template.split('$$')[0].replace("<div class='widget-plantransfercheck'>", ""));
            // 
            this.initEvent();
            ...
            //查询某个人计划列表
            this.user_id = "";
            // this.getPlanTransferCheck(this.config.pagenumber, this.config.pagesize, "");
            ...
            this.showLoading();
            // this.getAllpersonPlanTransferCheck(that.currentpage, this.config.pagesize, this.depid, this.step1_start, this.step1_end);
            this.getPlanTransferCheck(that.currentpage, this.currentpagesize, this.user_id);
    ...
    //查询某个人的所有计划
    getPlanTransferCheck(pagenumber, pagesize, region_id, depid, user_id, plan_begindate, plan_enddate) {
        ...
        $.ajax({
            ...
            type: "POST",
            url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + this.config.getPlanTransferCheckList,
            data: {
            ...
            success: this.getPlanTransferCheckListCallback.bind(this),
            error: function (data) {
    ...
    getPlanTransferCheckListCallback(results) {
        var that = this;
        if (results.code != 1) {
        ...
        $.each(results.result.rows, function (i, item) {
            ...
            html_trs_data += "<tr class=goto data-plan_id='" + item.plan_id + "' data-geometry='" + item.geometry + "' data-region_name='" + item.region_name + "' data-user_id='" + item.user_id + "' data-percent='" + item.percent + "'  data-region_id='" + item.region_id + "' data-region_name='" + item.region_name + "'  data-period_name='" + item.period_name + "'  data-period_id='" + item.period_id + "' data-plan_begindate='" + item.plan_begindate + "' data-plan_enddate='" + item.plan_enddate + "' data-plan_state='" + item.plan_state + "' data-check_state='" + item.check_state + "' data-isvalid='" + item.isvalid + "'><td>" + item.user_name + "</td><td>" + item.device_type_name + "</td><td>" + item.depname + "</td><td>" + item.period_name + "</td><td>" + item.region_name + "</td><td>" + item.plan_process_name + "</td><td>" + item.plan_begindate.split(" ")[0] + "</td><td>" + item.plan_enddate.split(" ")[0] + "</td><td>" + item.create_user + "</td><td>" + item.create_time + "</td><td><a class='operation' data-plan_id='" + item.plan_id + "' data-geometry='" + item.geometry + "' data-region_id='" + item.region_id + "' data-region_name='" + item.region_name + "'  data-device_type_id='" + item.device_type_id + "' >详情</a></td></tr>";
        ...
        if (results.result.rows.length < that.config.pagesize) {
            ...
            for (var i = 0; i < num; i++) {
                html_trs_blank += "<tr class=goto data-user_id='null'><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>";
            }
        ...
        this.domObj.off("click").on('click', 'tbody tr', (e) => {
            ...
            if ($(e.currentTarget).data("geometry").hasOwnProperty("paths")) {
                ...
                if ($(e.currentTarget).data("device_type_name") == "巡检线") {
                    this.map.centerAndZoom(mappolyline.getExtent().getCenter(), 9);
                } else {
                    this.map.centerAndZoom(mappolyline.getExtent().getCenter(), 10);
                }


                this.plandetail_polylinelayer_gl.clear();
                this.plandetail_polylinelayer_gl.add(new Graphic(mappolyline, this.setGraphSymbol("polyline")));
                ...
                this.plandetail_pointlayer_gl.clear();
                this.plandetail_pointlayer_gl.add(new Graphic(mapPoint, this.setGraphSymbol("point")));
    ...
    checkPlan(planid, ispass, checknote) {
        ...
        $.ajax({
            ...
            success: function (results) {
                ...
                //查询重新
                // this.getPlanTransferCheck(this.config.pagenumber, this.config.pagesize, this.user_id);
                this.getPlanTransferCheck(this.currentpage, this.config.pagesize, this.region_id, this.depid, this.user_id, this.plan_begindate, this.plan_enddate);
        ...
        this.domObj.find('.btn_confirm').off("click").on("click", function () {
            ...
            if (this.planid == "") {
                ...
                $.ajax({
                    ...
                    type: "POST",
                    url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + this.config.transferPlanTransferCheck,
                    data: {
                        "plan_id": this.planid,
                        "user_id": this.domObj.find('.users option:selected').val(),
                    ...
                    success: function (results) {
                        ...
                        // that.getAllpersonPlanTransferCheck(that.currentpage, this.currentpagesize, that.depid, that.step1_start, that.step1_end);
                        // that.getPlanTransferCheck(that.currentpage, this.currentpagesize, this.user_id);
                        this.getPlanTransferCheck(this.currentpage, this.currentpagesize, this.region_id, this.depid, this.user_id, this.plan_begindate, this.plan_enddate);
    ...
    initEvent5() {
        ...
        this.domObj.find(".pre").off("click").on("click", function () {
            ...
            if (that.Plan_list_currentpage - 1 > 0) {
                ...
                // that.getPlanTransferCheck(that.Plan_list_currentpage, that.config.pagesize, that.user_id);
        ...
        this.domObj.find(".dynamic_pagesize").off("change").on("change", function () {
            ...
            //重新查询
            this.currentpagesize = parseInt(this.domObj.find(".dynamic_pagesize").val());
            // that.getPlanTransferCheck(that.Plan_list_currentpage, this.currentpagesize, that.user_id);
            this.domObj.find(".halfpaneltable").height(this.domObj.find(".halfpaneltable").height());
        }.bind(this));
    ...
    //查询所有人的所有计划
    getAllpersonPlanTransferCheck(pagenumber, pagesize, depid, plan_begindate, plan_enddate) {
        ...
        $.ajax({
            ...
            success: this.getAllpersonPlanTransferCheckListCallback.bind(this),
            error: function (data) {
                this.toast.Show("服务端ajax出错，获取数据失败!");
                this.hideLoading();
            }.bind(this),
    ...
    getAllpersonPlanTransferCheckListCallback(results) {
        var that = this;
        if (results.code != 1) {
            that.toast.Show("查询巡检计划信息出错！");
            console.log(results.message);
    ...
    getAllPlanTransferCheck() {
        $.ajax({
            ...
            success: this.getAllPlanTransferCheckListCallback.bind(this),
            error: function (data) {
                this.toast.Show("服务端ajax出错，获取数据失败!");
            }.bind(this),
            dataType: "json",
    ...
    getAllPlanTransferCheckListCallback(results) {
        var that = this;
        if (results.code != 1) {
            that.toast.Show("查询巡检计划信息出错！");
            console.log(results.message);
    ...
    addPolygonGraphic(queryResult) {
        ...
        for (var i = 0, length = queryResult.result.rows.length; i < length; i++) {
            ...
            graphic.attr("id", "graphic" + i);
            this.plantransfercheckInfoGraphicLayer.add(graphic);
            //this.map.centerAt(point);

            //添加背景
            var point = polygon.getExtent().getCenter();
            var graphictextbg = new Graphic(point, this.setSymbol(row.regionname)[1].setOffset(0, -20), "");
            this.plantransfercheckInfoGraphicLayer.add(graphictextbg);
            //添加文字
            ...
            peopleTextSymbol.setFont(font);
            var graphicText = new Graphic(point, peopleTextSymbol, "");
            this.plantransfercheckInfoGraphicLayer.add(graphicText);
    ...
    addPlanTransferCheckInfoCallback(results) {
        ...
        if (results.code != 1) {
            ...
            this.plan_begindate = "";
            this.plan_enddate = "";
            if (this.plantransfercheckInfoGraphicLayer) {
                this.plantransfercheckInfoGraphicLayer.clear();
            }
            ...
            if (this.plantransfercheck_planpoint_clusterLayer != null) {
                this.map.removeLayer(this.plantransfercheck_planpoint_clusterLayer);
                this.plantransfercheck_planpoint_clusterLayer = null
            }
            this.regionid = "";
            //重新查询
            //进入步骤二
            ...
            //查询某个人计划列表
            // this.getPlanTransferCheck(this.config.pagenumber, this.config.pagesize, this.user_id);
            this.getPlanTransferCheck(that.currentpage, this.config.pagesize, this.region_id, this.depid, this.user_id, this.plan_begindate, this.plan_enddate);
    ...
    deletePlanTransferCheckCallback(results) {
        var that = this;
        if (results.code != 1) {
    ...
    updatePlanTransferCheckInfoCallback(results) {
        var that = this;
        if (results.code != 1) {
            ...
            //重新查询
            // this.getPlanTransferCheck(this.config.pagenumber, this.config.pagesize, this.user_id);
            that.getPlanTransferCheck(that.currentpage, this.config.pagesize, this.region_id, this.depid, this.user_id, this.plan_begindate, this.plan_enddate);
    ...
    Goto(url, layerid, geometry, type, where) {
        ...
        queryTask.execute(query, function (results) {
            ...
            if (results == null || results.features.length == 0) {
                if (this.plantransfercheck_devicepoint_clusterLayer != null) {
                    // this.plantransfercheck_planpoint_clusterLayer.clear();
                    this.map.removeLayer(this.plantransfercheck_devicepoint_clusterLayer);
                    this.plantransfercheck_devicepoint_clusterLayer = null;
                    return;
                }
            }
    ...
    addQuerydataClusters(querydata, type) {
        ...
        if (this.plantransfercheck_devicepoint_clusterLayer != null) {
            // this.plantransfercheck_devicepoint_clusterLayer.clear();
            this.map.removeLayer(this.plantransfercheck_devicepoint_clusterLayer);
            this.plantransfercheck_devicepoint_clusterLayer = null
        }
        this.plantransfercheck_devicepoint_clusterLayer = new ClusterLayer({
            "basemap": this.map,
            "data": photoInfo.data,
            "distance": 10,
            "id": "plantransfercheck_planpoint_clusters",
            "labelColor": "#fff",
            "labelOffset": 10,
        ...
        this.plantransfercheck_devicepoint_clusterLayer.setRenderer(renderer);

        this.map.addLayer(this.plantransfercheck_devicepoint_clusterLayer);
    ...
    addClusters(resp) {
        ...
        this.plantransfercheck_planpoint_clusterLayer = new ClusterLayer({
            "basemap": this.map,
            "data": photoInfo.data,
            "distance": 10,
            "id": "plantransfercheck_planpoint_clusters",
        ...
        var blue = new PictureMarkerSymbol(this.config.MarkPictureSymbol, 32, 32).setOffset(0, 15);
        renderer.addBreak(0, 10000, blue);//范围放大点，用一种图标渲染
        this.plantransfercheck_planpoint_clusterLayer.setRenderer(renderer);

        this.map.addLayer(this.plantransfercheck_planpoint_clusterLayer);
    ...
    destroy() {
        if (this.plantransfercheckInfoGraphicLayer) {
            this.map.removeLayer(this.plantransfercheckInfoGraphicLayer);
            this.plantransfercheckInfoGraphicLayer.clear();
        }
        ...
        if (this.plandetail_polylinelayer_gl) {
            this.map.removeLayer(this.plandetail_polylinelayer_gl);
            this.plandetail_polylinelayer_gl.clear();
        }
        if (this.plandetail_pointlayer_gl) {
            this.map.removeLayer(this.plandetail_pointlayer_gl);
            this.plandetail_pointlayer_gl.clear();
        }

        if (this.plantransfercheck_planpoint_clusterLayer) {
            this.map.removeLayer(this.plantransfercheck_planpoint_clusterLayer);
        }
        if (this.plantransfercheck_devicepoint_clusterLayer) {
            this.map.removeLayer(this.plantransfercheck_devicepoint_clusterLayer);
        }
