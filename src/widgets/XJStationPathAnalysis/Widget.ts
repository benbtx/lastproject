...
class XJPathAnalysis extends BaseWidget {
    ...
    initInterface() {
        ...
        switch (this.userType) {
            ...
            case "departmentadmin":
                ...
                this.requestUserInfo(0, 0, AppX.appConfig.groupid, AppX.appConfig.departmentid, this.initUserList.bind(this));
                break;
        }
        // //初始化部门列表
    ...
    */
    ...
    getQueryParms() {
        ...
        var workerNameId = this.domObj.find("select.userName").find("option:selected").val();
        ...
        queryParms.push(workerNameId, date, dateNext, startTime, endTime);
        return queryParms;
    }

    //获取轨迹分析结果
    requestPathAnalysisResult(pageindex, pagesize, workerId, date, startTime, endTime, callback) {
    ...
    initDataTable(result) {
        ...
        for (var i = 0, length = rows.length; i < length; i++) {
            ...
            var time = rows[i].check_time.split(" ")[1];//gps定位时间
            ...
            var gpsType = "";
            if (rows[i].gps_type == 0) {
                gpsType = "人巡";
            } else if (rows[i].gps_type == 1) {
                gpsType = "车巡";
            }
            data.push(prepoint, point, time, distance, needTime, speed, star, precision, gpsType);
            var index = 0;
    ...
    addPathToMap(result) {
        ...
        for (var i = 0, length = rows.length; i < length; i++) {
            ...
            //添加轨迹
            var pathPoint = [];
            if (i == 0 || rows[i].gps_type == rows[i - 1].gps_type) {
                pathPoint.push(x);
                pathPoint.push(y);
                linePath.push(pathPoint);
                //处理最后一段
                if (i == rows.length - 1) {
                    if (rows[i - 1].gps_type === 0) {
                        symbol = this.symbol.lineSymbol;
                    } else if (rows[i - 1].gps_type === 1) {
            ...
            } else if (rows[i].gps_type !== rows[i - 1].gps_type) {
    ...
    itemClick(event, dbclick) {
        ...
        this.hilightPath(prePoint, point, false);
    }
    dbitemClick(event, dbclick) {
        ...
        var prePoint = $(event.currentTarget).attr("prepoint");
        var point = $(event.currentTarget).attr("point");
        this.hilightPath(prePoint, point, true);
    }
    //高亮选中点
    hilightPath(prepoint, point, dbclick) {
        //清除之前的线
        this.highlightPathGraphicLayer.clear();
        //添加目前需要高亮的线
