...
export = AdressQuery;
class AdressQuery extends BaseWidget {
    baseClass = "adressQuery";
    ...
    adressQueryInit() {
        ...
        if (this.AppX.appConfig.gisResource.pipe.config.length == 0) {
            ...
            //查询按钮 添加绑定事件
            $(".adressQuery-queryBtn").bind("click", this.queryBtnCallback.bind(this));
            var maxLayerCount = layers.filter(item => { return item.urlindex == 0 }).length;
            this.limitSelect(".adressQuery-layerName", maxLayerCount);
        }
    }
    ...
    addOptionToDrapdow(layers) {
        ...
        for (var i = 0, length = layers.length; i < length; i++) {
            ...
            $(option).appendTo($(".adressQuery-layerName"));
    ...
    queryBtnCallback(event) {
        ...
        var inputObj = $(".adressQuery input");
        if (inputObj.val() == "") {
            inputObj.attr("placeholder", "不能为空！");
            return;
        }
        if ($(".adressQuery-queryBtn").text() == '查询中...') {
            return;
        }
        $(".adressQuery-queryBtn").text('查询中...');
        var SQLWhere: string = inputObj.val();
        SQLWhere = "ADDR like" + "'%" + SQLWhere + "%'";

        var selectedObj = $(".adressQuery-layerName").find("option:selected");
        //获取选中图层的id
