...
class InfoWindows extends Base {
    private map: Map = null;
    private windows: Array<InfoWindow> = []//浮动窗体集合
    private mapEvents = [];
    private initWidth: '230px';
    private initHeight: '125px';
    private infoWindowContainner = "<div class=\"infowindowx-containner\"/>";
    private inforWindowxTemplate = "<div class=\"infoWindowx-anchor\"><div class=\"infoWindowx\"><div class=\"infoWindowx-PopupWrapper\"><div class=\"infoWindowx-sizer\"><div class=\"infoWindowx-titlePane\"><div class=\"infoWindowx-title\"></div><div class=\"infoWindowx-titleButton close\" title=\"关闭\"></div></div></div><div class=\"infoWindowx-sizer content\"><div class=\"infoWindowx-contentPane\"></div></div><div class=\"infoWindowx-pointer bottomLeft\"></div></div></div></div>"
    /**
    * (方法说明)构造函数
    * @method (方法名)
    * @for (所属类名)
    ...
    */
    ...
    public AddNewInfoWindow(setting: {
        id: string,
        title: string,
        content: string,
        point: Point
        ...
        if (index != -1) return;
        //构建浮动窗体容器,保证浮动窗体在panel下面
        var inforContainner = $('.infowindowx-containner');
        if (inforContainner.length == 0) {
            inforContainner = $(this.infoWindowContainner).insertBefore($("#panelContainer"));
        }
        //构建浮动窗体
        var infowindow = $(this.inforWindowxTemplate).appendTo(inforContainner);
        var screenPoint = this.map.toScreen(setting.point);
        infowindow.css("left", screenPoint.x + "px");
        infowindow.css("top", screenPoint.y + "px");
        ...
        this.windows.push({ id: setting.id, domobj: infowindow, point: setting.point });
    }

    public ShowInfoWindow(ptid: string) {
        var index = _.findIndex(this.windows, function (infowindow) { return infowindow.id == ptid });
        if (index != -1) {
            this.windows[index].domobj.appendTo($('.infowindowx-containner')).show();
        }
    }
    ...
    */
    public Clean() {
        ...
        $('.infowindowx-containner').off().remove();
    }
    ...
    private mapExtentChange(event) {
        this.windows.forEach(infonwindow => {
            ...
            infonwindow.domobj.css("left", screenPoint.x + "px");
            infonwindow.domobj.css("top", screenPoint.y + "px");
        });
    }
    ...
    private mapPan(event) {
        this.windows.forEach(infonwindow => {
            ...
            infonwindow.domobj.css("left", left + "px");
            infonwindow.domobj.css("top", top + "px");
        });
    }

    private zoomStartEvent(event) {
        this.windows.forEach(infonwindow => {
            infonwindow.domobj.find('.infoWindowx').hide();
        });
    }
    ...
    private zoomEndEvent(event) {
        ...
        this.windows.forEach(infonwindow => {
            infonwindow.domobj.find('.infoWindowx').show();
        });
    }
}