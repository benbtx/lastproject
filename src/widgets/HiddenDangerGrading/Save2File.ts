...
function save2CSV(data: any, fileName?: string) {
    ...
    var name = "隐患定级列表";
    name += '-' + getDataTime();
    result += "隐患类型,隐患说明,上报人员,地址,上报时间,公司名称";

    for (var i = 0, j = data.length; i < j; i++) {

        // result += "\n" + pureData.join(',');
        result += "\n" + data[i].trouble_type_name + "," + data[i].handle_before_notes.trim() + "," + data[i].username + "," + data[i].address + "," + data[i].uploadtime + "," +data[i].company_name;
    }
