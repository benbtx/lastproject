...
export = DeviceTimeStatistics;

class DeviceTimeStatistics extends BaseWidget {
    baseClass = "widget-devicetimestatistics";
    toast = null;
    popup = null;
    loadWait = null;
    ...
    initEvent() {
        ...
        this.domObj.find('.btn_search').on("click", function () {
            ...
            //查询分析结果
            this.getdevicetimestatistics();
        }.bind(this));

        //打印
        this.domObj.find('.btn_print').on("click", function () {
            (<any>$(".devicetimestatistics-list")).print({
                globalStyles: false,
                mediaPrint: false,
                stylesheet: "widgets/devicetimestatistics/css/style.css",
                iframe: true,
        ...
        this.domObj.find('.btn_export').on("click", function () {
            mothedexport_devicetimestatistics();
        }.bind(this));
     ...
     */
    addtitle() {
        var htmStr = '<tr><th>员工姓名</th><th>部门</th> <th>巡检周期</th>';
        for (var i = 0; i < this.typeselect.length; i++) {
            htmStr += '<th>' + this.config.device_name_selects[this.typeselect[i] + ""] + '</th>'
        }
        htmStr += ' <th>完成比例</th></tr>';
        this.domObj.find('.deviceplantime_title').empty().append(htmStr);


        var htmStr_colgroup = '<col width="80"> <col width="100"> <col width="100">';
        for (var i = 0; i < this.typeselect.length; i++) {
            htmStr_colgroup += ' <col width="120">'
        }
        htmStr_colgroup += '<col width="100" >';
    ...
    getdevicetimestatistics() {
        ...
        this.typeselect = []
        $('.deviceplantype option:selected').each(function () {
            that.typeselect.push($(this).val());
        });

        this.peridselect = []
        $('.deviceperiodid option:selected').each(function () {
            that.peridselect.push($(this).val());
        });
        if (this.typeselect.length == 0) {
            this.toast.Show("设备类型必须选择");
            return;
        }
        this.addtitle();
        ...
        $.ajax({
            ...
            success: this.getdevicetimestatisticsCallback.bind(this),
            error: function (data) {
                this.loadWait.Hide();
    ...
    getdevicetimestatisticsCallback(results) {
        this.loadWait.Hide();
        var that = this;
        if (results.code != 1) {
            that.toast.Show(results.message);
        ...
        for (var i = 0; i < results.result.rows.length; i++) {
            ...
            item.completepercent = (pointper[1] == 0 ? 0 : (pointper[0] / pointper[1]) * 100);
            if (sameuser == null) {
        ...
        for (var i = 0; i < userinfos.length; i++) {
            ...
            for (var j = 0; j < count; j++) {
                ...
                detailStr += "<td>" + detobj.completepercent.toFixed(2) + "%" + "</td>"
                if (j == 0) {
        ...
        this.domObj.find(".devicetimestatistics").empty().append(this.html);
        if (this.iszgs) {
    ...
    */
    ...
    getPlanTypeListCallback(results) {
        ...
        this.domObj.find(".deviceplantype").empty().append(strplantype);
        $('.deviceplantype').multiselect({
            nonSelectedText: '请选择',
    ...
    getPeriodListCallback(results) {
        ...
        this.domObj.find(".deviceperiodid").empty().append(strperiod);
        $('.deviceperiodid').multiselect({
            nonSelectedText: '请选择',
            allSelectedText: '全被选中',
            nSelectedText: "个被选中",
            numberDisplayed: 1,
