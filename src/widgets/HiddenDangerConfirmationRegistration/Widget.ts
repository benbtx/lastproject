...
export = HiddenDangerConfirmationRegistration;

class HiddenDangerConfirmationRegistration extends BaseWidget {
    baseClass = "widget-hiddendangerconfirmationregistration";
    map = null;
    toast = null;
    photowall = null;
    popup = null;
    ...
    initHtml() {
        ...
        this.setHtml(html);
        this.ready();
        this.getHiddenDanger(this.config.pagenumber, this.config.pagesize, this.trouble_typeid, this.pdaid, this.userid, this.process_id);
    }
    ...
    initEvent() {
        ...
        this.domObj.find('.btn-search').off("click").on("click", function () {
            ...
            if (this.showtype == 0) {
                ...
                this.getHiddenDanger_icon(this.config.pagenumber, 1000000, this.trouble_typeid, this.pdaid, this.userid, this.process_id);
            }
        ...
        this.domObj.find('.save2csv').off("click").on("click", function () {
            // var pong = new Howl({ src: ['http://localhost:3000/widgets/HiddenDangerConfirmationRegistration/1.mp3'] });
            // pong.play();
        ...
        this.domObj.off("click").on('click', 'tbody tr', (e) => {
            ...
            // that.map.infoWindow.setTitle("<div class='HiddenDangerConfirmationRegistration-id' id=" + $(e.currentTarget).data("id") + ">标题</div>");
        ...
        this.domObj.find(".dynamic_pagesize").off("change").on("change", function () {
            ...
            this.getHiddenDanger(this.config.pagenumber, this.currentpagesieze, this.trouble_typeid, this.pdaid, this.userid, this.process_id);
    ...
    getHiddenDanger(pagenumber, pagesize, trouble_typeid, pdaid, userid, process_id) {
    ...
    getHiddenDangerListCallback(results) {
        ...
        this.domObj.find(".operation").off("click").on("click", function (e) {
            ...
            popup.setTitle("<div class='HiddenDangerConfirmationRegistration-id'>标题</div>");
            ...
            if ($(e.currentTarget).data("yhyp") != "") {
                if ($(e.currentTarget).data("yhyp").indexOf(',') >= 0) {
                    $(e.currentTarget).data("yhyp").split(',').forEach(
                        i => stryhyp += "<image class='hiddedangerinfo_audio'  src='./widgets/HiddenDangerConfirmationRegistration/images/audio.png' data-yhyp='" + AppX.appConfig.xjapiroot.substr(0, AppX.appConfig.xjapiroot.length - 3) + i + "'/>"
                    );
                } else {
                    stryhyp = "<image class='hiddedangerinfo_audio' src='./widgets/HiddenDangerConfirmationRegistration/images/audio.png' data-yhyp='" + AppX.appConfig.xjapiroot.substr(0, AppX.appConfig.xjapiroot.length - 3) + $(e.currentTarget).data("yhyp") + "'/>";
                }
            ...
            //定义隐患挂接片区
            $(".btn-confirmationregistration").off("click").on("click", function (e) {

                //弹出popup
                this.popup.setSize(600, 350);
                var Obj = this.popup.Show("隐患确认登记", this.template.split('$$')[1]);
                Obj.submitObj.off("click").on("click", function () {
                    ...
                    $.ajax({
                        ...
                        success: this.addconfirmationregistrationCallback.bind(this),
                        error: function (data) {
                            this.toast.Show("服务端ajax出错，获取数据失败!");
                        }.bind(this),
                        dataType: "json",
        ...
        $(".confirmationregistration").off("click").on("click", function (e) {


            this.getAllPlanRegion_contain();
            this.trouble_id = $(e.currentTarget).data("id");
            //弹出popup
            this.popup.setSize(600, 220);
            var Obj = this.popup.Show("隐患确认登记", this.template.split('||')[6]);
            Obj.submitObj.off("click").on("click", function () {
                var region = Obj.conObj.find('.confirmationregistration_region option:selected');
                ...
                $.ajax({
                    ...
                    type: "POST",
                    url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + this.config.addConfirmRegistration,
                    data: {
    ...
    addconfirmationregistrationCallback(results) {
    ...
    getAllPlanRegion_containListCallback(results) {
        ...
        var region = $(".confirmationregistration_region").empty();
        var str_region = "";
    ...
    getHiddenDanger_icon(pagenumber, pagesize, trouble_typeid, pdaid, userid, process_id) {
        this.currentpage = pagenumber || this.config.pagenumber;
        // this.currentpagesieze = pagesize || this.config.pagesize;
        $.ajax({
            headers: {
