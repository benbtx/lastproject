...
function save2CSV(data: any, fileName?: string) {
    ...
    var name = "隐患列表";
    name += '-' + getDataTime();
    result += "设备名称,隐患类型,隐患备注,上报人员,是否处理登记,隐患是否清除,线下是否清除,地址,发现时间";

    for (var i = 0, j = data.length; i < j; i++) {

        // result += "\n" + pureData.join(',');
        result += "\n" + data[i].padname + ","  + data[i].trouble_type_name + "," + data[i].notes + "," + data[i].username + "," + (data[i].register_state==0?"否":"是") + ","+(data[i].handle_trouble_clear==0?"未清除":"已清除") + "," + (data[i].line_trouble_clear==0?"未清除":"已清除") + "," + data[i].address + "," + data[i].uploadtime;
    }
