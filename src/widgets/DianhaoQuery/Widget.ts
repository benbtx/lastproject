...
class DianhaoQuery extends BaseWidget {
    ...
    dianhaoQueryInit() {
        this.domObj.find("input").on("keydown", this.onInputKeyDown.bind(this));
        if (this.AppX.appConfig.gisResource.pipe.config.length == 0) {
            ...
            //查询按钮 添加绑定事件
            $(".dianhaoQuery-queryBtn").bind("click", this.queryBtnCallback.bind(this));
            var maxLayerCount = layers.filter(item => { return item.urlindex == 0 }).length;
            this.limitSelect(".dianhaoQuery-layerName", maxLayerCount);
        }
    }
    ...
    queryBtnCallback(event) {
        ...
        var inputObj = $(".dianhaoQuery input");
        if (inputObj.val() == "") {
            inputObj.attr("placeholder", "不能为空！");
            return;
        }
        if ($(".dianhaoQuery-queryBtn").text() == '查询中...') {
            return;
        }
        $(".dianhaoQuery-queryBtn").text('查询中...');
        var SQLWhere: string = inputObj.val();
        SQLWhere = "OLDNO like" + "'%" + SQLWhere + "%'";

        var selectedObj = $(".dianhaoQuery-layerName").find("option:selected");
        //获取选中图层的id
