...
export = HiddenDangerDeviceManagement;

class HiddenDangerDeviceManagement extends BaseWidget {
    baseClass = "widget-hiddendangerdevicemanagement";
    ...
    initHtml() {
        ...
        this.setHtml(html);
        this.ready();
        this.getHiddenDangerDevice(this.config.pagenumber);
    }
    ...
    getHiddenDangerDeviceListCallback(results) {
        var that = this;
        if (results.code != 1) {
        ...
        $.each(results.result.rows, function (i, item) {
            ...
            html_trs_data += "<tr class=goto  data-id='" + item.device_type_id + "' data-name='" + item.name + "' data-type='" + item.type + "' data-note='" + item.note + "' ><td>" + item.name + "</td><td>" + lx + "</td></tr>";
            //刷新infowindow
    ...
    initEvent() {
        ...
        this.domObj.find('.btn_add').on("click", function () {
            ...
            //验证
            (<any>$('#hiddendangerdevicemanagement_addpopup')).bootstrapValidator();
            ...
            Obj.submitObj.off("click").on("click", function () {
                ...
                $.ajax({
                    ...
                    type: "POST",
                    url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + this.config.addHiddenDangerDevice,
                    data: {
                        "name": name.val(),
                        "type": type.val(),
                    ...
                    success: this.addHiddenDangerDeviceCallback.bind(this),
                    error: function (data) {
        ...
        this.domObj.find('.btn_edit').on("click", function () {
            ...
            // Obj.conObj.find('.id').val(this.hiddendangerdeviceid);
            Obj.conObj.find('.name').val(this.name);
            Obj.conObj.find('.type').val(this.type);


            //验证
            (<any>$('#hiddendangerdevicemanagement_updatepopup')).bootstrapValidator();
            ...
            Obj.submitObj.off("click").on("click", function () {
                ...
                $.ajax({
                    ...
                    type: "POST",
                    url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + this.config.updateHiddenDangerDevice,
                    data: {
                        "id": this.hiddendangerdeviceid,
                        "name": name.val(),
                        "type": type.val(),

                        // "default_level": strhiddendangerlevel,
                    },
                    success: this.updateHiddenDangerDeviceCallback.bind(this),
                    error: function (data) {
                        this.toast.Show("服务端ajax出错，获取数据失败!");
                    }.bind(this),
                    dataType: "json",
        ...
        this.domObj.find('.btn_delete').on("click", function () {
            ...
            Obj.submitObj.off("click").on("click", function () {
                $.ajax({
                    ...
                    type: "POST",
                    url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + this.config.deleteHiddenDangerDevice + this.hiddendangerdeviceid,
                    data: {
                        "id": this.hiddendangerdeviceid,
                        // f: "pjson"
                    },
                    success: this.deleteHiddenDangerDeviceCallback.bind(this),
                    error: function (data) {
                        this.toast.Show("服务端ajax出错，获取数据失败!");
                    }.bind(this),
                    dataType: "json",
        ...
        this.domObj.off("click").on('click', 'tbody tr', (e) => {
            if ($(e.currentTarget).data("id") == null) {
                ...
                this.hiddendangerdeviceid = $(e.currentTarget).data("id");
        ...
        this.domObj.find(".pre").off("click").on("click", function () {
            if (this.currentpage - 1 > 0) {
                ...
                this.getHiddenDangerDevice(this.currentpage);
    ...
    getHiddenDangerDevice(pagenumber) {
        ...
        $.ajax({
            ...
            type: "POST",
            url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + this.config.getHiddenDangerDeviceList,
            data: {
            ...
            success: this.getHiddenDangerDeviceListCallback.bind(this),
            error: function (data) {
                this.toast.Show("服务端ajax出错，获取数据失败!");
            }.bind(this),
            dataType: "json",
