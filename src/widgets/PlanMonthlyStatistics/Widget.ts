...
export = PlanMonthlyStatistics;

class PlanMonthlyStatistics extends BaseWidget {
    baseClass = "widget-planmonthlystatistics";
    toast = null;
    popup = null;
    loadWait = null;
    ...
    //初始化日期控件
    initDateWidget() {
        $.jeDate(".widget-planmonthlystatistics  input.date", {
            format: 'YYYY-MM', //日期格式  
            isinitVal: true,
            initAddVal: { MM: "0" },
            maxDate: $.nowDate(0),
        ...
        this.queryDate = this.domObj.find("input.date").val();
    }
    ...
    initEvent() {
        ...
        this.domObj.find('.btn_search').on("click", function () {
            ...
            //查询分析结果
            this.getPlanMonthlyStatistics();
        }.bind(this));

        //打印
        this.domObj.find('.btn_print').on("click", function () {
            (<any>$(".planmouthlystatistics-list")).print({
                globalStyles: false,
                mediaPrint: false,
                stylesheet: "widgets/PlanMonthlyStatistics/css/style.css",
                iframe: true,
        ...
        this.domObj.find('.btn_export').on("click", function () {
            mothedexport_monthlystatistics();
        }.bind(this));
    }
     ...
     * 获取月报统计信息
     */
    getPlanMonthlyStatistics() {
        this.queryDate = this.domObj.find("input.date").val();
        this.loadWait.Show("正在查询数据，请耐心等待...", this.domObj);
        $.ajax({
            headers: this.getHeaders(1),
            type: "POST",
            url: AppX.appConfig.xjapiroot.replace(/\/+$/, "") + this.config.getMonthlyStatistics,
            data: {
                "companyid": this.companyid,
                "depid": this.depid,
                "start_date": this.queryDate,
                "end_date": "",
                "static_type": 2
            },
            success: this.getPlanMonthlyStatisticsCallback.bind(this),
            error: function (data) {
                this.loadWait.Hide();
    ...
    getPlanMonthlyStatisticsCallback(results) {
        this.loadWait.Hide();
        var that = this;
        if (results.code != 1) {
            that.toast.Show(results.message);
        ...
        this.domObj.find(".planmonthlystatistics").empty().append(this.html);
        if (this.iszgs) {
            this.domObj.find(".cxgs").text("公司：" + this.domObj.find(".company option:selected").text());
        }
        var queryDateSpan = this.queryDate.replace("/", "年") + "月";
        this.domObj.find(".cxqj").text("查询时间：" + queryDateSpan);
        var myDate = new Date();
