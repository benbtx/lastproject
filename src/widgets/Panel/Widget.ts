...
class Panel extends BaseWidget {
    ...
    constructor(settings, panelOpinions: {
        ...
        this.panelcount = panelOpinions.panelcount;
    }
    ...
    startup() {
        ...
        this.setHtml(_.template(this.template)({
            height: this.mainContainerObj.height() - this.panelHeightDelta,
    ...
    interactable() {
        ...
        windowObj.on('resize.panel', function () {
            ...
            if (domObjHeight > mainContainerHeight - this.panelHeightDelta) {
                // 面板高度小于临界值时停止改变其高度
                if (mainContainerHeight - this.panelHeightDelta > this.resizeMinHight) {
                    that.domObj.height(mainContainerHeight - that.panelHeightDelta);
                }
            }
            // 如果面板移出界面,将其拉回
            if (that.domObj.offset().left + domObjWidth > windowObj.width()) {
                that.domObj.css({
                    top: this.top + "px",
                    left: windowObj.width() - domObjWidth - this.right + "px"
                });
            }
        }.bind(this));
    ...
    offset(panelcount) {
        var top = parseInt(this.domObj.css("top"));
        var right = parseInt(this.domObj.css("right"));
        this.domObj.css({ "top": top + panelcount * this.offsets, "right": right + panelcount * this.offsets });
